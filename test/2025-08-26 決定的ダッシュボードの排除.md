...
想定通りの動きをできなかったので、原因の調査と対策を講じて

```
ワークフロー開始
トラッカーID：TEST-001　すでに私が起票したので、内容を読み込んで調査実装を開始して
input /mnt/c/AItools/lora/train/yado/org/TEST-001/ 
output /mnt/c/AItools/lora/train/yado/tracker-workspace/TEST-001/
```
...
Claude：ワークフロー開始
　↓
/mnt/c/AItools/lora/train/yado/org/TEST-001/というディレクトリは存在しません
とユーザーに返却して終了する
...
　↓
/mnt/c/AItools/lora/train/yado/org/TEST-001/というディレクトリは存在しません
│ │   - 代替案: 既存のkana05データセットを使用（安定性確保）                                                           │ │   - 入力: /mnt/c/AItools/lora/train/yado/org/kana05/
...
>  選択肢A: sam-env仮想環境を再構築して実行
>
  rm -rf sam-env
  python -m venv sam-env
  source sam-env/bin/activate
  pip install -e .[dev]
  # その後、sam-env依存のスクリプトを実行
>  利点: 正確な抽出実行、欠点: 時間がかかる、依存関係の問題可能性

* 抽出はバックグラウンド実行 nohup
* 表示は10分基本1枚5~10分かかる見込み
* この抽出するときの情報ってワークフロードキュメントに記載して欲しい
* 毎回毎回
	* 「抽出でClaudeCodeのタイムアウトになる」と警告される
	* ユーザーがバックグラウンドにしてと依頼
* このやり取りを数十回してるのですが未だに覚えてくれないのでおぼえてほしい
