...
一旦全部厳密化して、いっこづつ指定して実行することはユーザーはしない、最初の依頼のときに「input outputしてするかしないか」のどっちだと思う

また、以下のパスが存在しないパスの場合もエラーを出してほしい
```
    --input /mnt/c/AItools/lora/train/{作者名}/org/{作品名}/ \
    --output /mnt/c/AItools/lora/train/{作者名}/tracker-workspace/{TRACKER_ID}/
```
