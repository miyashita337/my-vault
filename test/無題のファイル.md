...
ワークフローではフルパスを必ず指定してます
なのでそのときに「作者名」「作品名」は判明してる。なので私は必ず以下を指定します

```

input   /mnt/c/AItools/lora/train/{作者名}/org/{作品名}
output /mnt/c/AItools/lora/train/{作者名}/tracker-workspace/

```

でも指定してなくても動作する場合がある

また、こちらの落ち度でフルパスを毎回指定しないとだめな場合があるのにもかかわらず、自動的に通ってしまうのが問題、それはつまり意図しない挙動に繋がる可能性がある

例）
```
[ユーザー]ワークフローを実行して
[local/claude]..実行中、出力しました
[ユーザー]確認したけど出力してない
[local/claude]..実行中、出力しました
[ユーザー]もう一度確認したけど出力してない
（ここで）
```


指定してない場合はエラーが出るようにしてほしい
このinput output部分はなんとなくではなく厳しく見てほしい
