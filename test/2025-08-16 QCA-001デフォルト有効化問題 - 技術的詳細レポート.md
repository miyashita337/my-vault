...
--------------
  ...
  誤解を解く重要ポイント:
  - ❌ 各トラッカーごとに異なるコミットに戻す必要はない
  - ✅ QCA-001実装前の1つのコミットに戻すだけで良い

  🎯 正しい作業フロー

  # ステップ1: QCA-001実装前のコミットに一度だけ戻す
  git checkout 7fe93f6  # 2025-08-10 11:29:36より前のコミット

  # ステップ2: extract_character.pyのdefault値を確認・修正
  # default=False になっていることを確認

  # ステップ3: 各トラッカーを順次処理（33回）
  for tracker in [CI-INTEGRATION-001, COMPOSITE-BASELINE-001, ...]; do
      # 3-1: 抽出プログラム実行（真のベースライン）
      python features/extraction/commands/extract_character.py \
          input_dir/ -o tracker-workspace/${tracker}/extraction/ --batch

      # 3-2: 品質チェック実行
      python tools/core/unified_quality_checker.py ${tracker}

      # 3-3: ダッシュボード生成
      python features/common/dashboard_generator.py ${tracker}

      # 3-4: 統計分析（存在する場合）
      python tools/progress_tracker/qcc023_enhanced_cohens_d.py ${tracker}
  done
  ...
  | 項目       | ユーザーの認識       | 実際の作業       |
  |----------|---------------|-------------|
  | コミット戻し回数 | 31回（各トラッカーごと） | 1回のみ        |
  | 作業の複雑さ   | 非常に複雑         | シンプル        |
  | コミット管理   | 31個の異なるコミット管理 | 1つのコミットで全対応 |
  | 実現可能性    | 困難（コミット特定が複雑） | 現実的         |

  ⚠️ 追加の技術的考慮事項

これは再現とうよりも、以前のトラッカーよりも少しマシにして再計算し直すというかんじですかね？
features/extraction/commands/extract_character.py自体だけでなく、この周辺ソースコードすべてが段階的にコミットを戻して実行しないと行けないけど、それだとかなり複雑すぎるので
