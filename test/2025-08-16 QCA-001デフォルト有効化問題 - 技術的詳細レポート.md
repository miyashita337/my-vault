...
> 5. エラー処理とフォールバック
  - 比較対象トラッカーが存在しない場合の処理
>  - extraction_result.jsonが不完全な場合の対応
  - サンプルサイズが小さすぎる場合の警告

エラー処理は、
* エラー文言を詳しく出力して(後でClaudeが理解できやすいように)
* トラッカーが存在しない場合はエラーでそのまま止めて
	* ユーザーが間違ったトラッカーIDを指定した可能性があるので
* extraction_result.jsonが不完全な場合の対応
	* extraction_result.jsonを作り直す
	* 対象となる抽出画像がそもそも存在してなかったらエラーでそのまま止めて
* サンプルサイズが小さすぎる場合の警告
	* 警告はダッシュボードに出しつつそのまま続行
