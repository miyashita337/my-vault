...
>    - 困難姿勢処理でリトライ時に既に前処理済みファイルを再処理
  ...
  3. 処理フロー
>  元画像 → 漫画前処理 → preprocessed_manga_[filename]
  ↓ (リトライ時)
  preprocessed_manga_[filename] → 再処理 → preprocessed_manga_preprocessed_manga_[filename]
>  影響:
  - 入力ディレクトリにファイルが蓄積
>  - ディスク使用量増加
  - ファイル管理の複雑化
>  解決が必要な箇所:
  features/evaluation/utils/difficult_pose.pyの前処理ロジックで、既存のプレフィックスを検出して重複を防ぐ必要があります。

 エフェクト線除去、境界平滑化、モザイク処理
 モザイク処理はしないでください
 私が「モザイク」といったのは、境界値や
